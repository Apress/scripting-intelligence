...
# to get HTML::FullSanitizer from Rails ActionController:
...
class HtmlXhtmlResource < TextResource
  def initialize source_uri=''
    ...
    @title = doc.xpath('//title').collect {|h| h.inner_text.strip}.join(' ')
    process_text_semantics!(@plain_text)
  end
end
...
require 'pp'
#tr1 = PlainTextResource.new("test_data/wikipedia_Hillary Rodham Clinton.txt")
#tr2 = PlainTextResource.new("test_data/wikipedia_Barack_Obama.txt")
#pp tr2

#tr = RssResource.get_entries('http://feeds.boingboing.net/boingboing/iBag')
#pp tr

tr = AtomResource.get_entries('http://markwatson.com/blog/atom.xml')
pp tr
